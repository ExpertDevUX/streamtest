...
install_ffmpeg() {
    ...
    if [[ "$OS" == *"Ubuntu"* ]] || [[ "$OS" == *"Debian"* ]]; then
        # Add FFmpeg PPA for latest version
    ...
    server {
        ...
        application live {
            ...
            exec ffmpeg -i rtmp://localhost/live/$name -async 1 -vsync -1
                        -c:v libx264 -c:a aac -b:v 2500k -b:a 128k -vf "scale=1920:1080" -tune zerolatency -preset veryfast -crf 23 -f flv rtmp://localhost/hls/$name_1080p
                        -c:v libx264 -c:a aac -b:v 1000k -b:a 128k -vf "scale=1280:720" -tune zerolatency -preset veryfast -crf 23 -f flv rtmp://localhost/hls/$name_720p
                        -c:v libx264 -c:a aac -b:v 500k -b:a 96k -vf "scale=854:480" -tune zerolatency -preset veryfast -crf 23 -f flv rtmp://localhost/hls/$name_480p;
        }
